...
class Database:
    ...
    def get_redeemed_code_for_user(self, code: str, tg_user_id: int) -> Optional[sqlite3.Row]:
        with closing(self.connect()) as conn:
            return conn.execute(
                """
                SELECT rc.*, e.title, e.grade_level, e.subject, e.category, e.duration_minutes, e.description, e.answer_key, e.answer_instructions
                FROM redeem_codes rc
                JOIN exams e ON e.id = rc.exam_id
...
def school_exam_info_text() -> str:
    return (
        ...
        "👤 Shaxsiy ma'lumotlar — sinfingiz, telefoningiz, ishlatgan kodlaringiz va oxirgi natijalaringiz.\n\n"
...
def help_text() -> str:
    lines = [
        ...
        "2) Sinfingizni tanlang.",
        "3) Full mock yoki Fanlar bo'limidan fan tanlang.",
        "4) Bir martalik tekin testni ishlating yoki kod kiriting.",
        "5) Testni ishlab bo'lgach ✅ Test tekshirish bo'limiga kiring.",
        "6) Kodni yuboring, so'ng javoblarni yuboring.",
        "7) Natija foiz, baho va tahlil bilan chiqadi.",
        "8) Shaxsiy ma'lumotlar bo'limida ishlatgan kodlar va oxirgi natijalarni ko'rasiz.",
    ]
...
def profile_text(user: sqlite3.Row, attempts: list[sqlite3.Row], codes: list[sqlite3.Row]) -> str:
    lines = [
        ...
        f"Sinf: {escape_html_text(grade_label(user['target_grade']))}",
...
def export_users_xlsx() -> Path:
    ...
    for idx, row in enumerate(DB.list_users(), start=1):
        rows.append([idx, row["tg_user_id"], row["username"] or "-", row["full_name"] or "-", row["phone"] or "-", grade_label(row["target_grade"]), row["attempts_count"], row["registered_at"]])
    return build_xlsx_file("users_export.xlsx", ["#", "TG ID", "Username", "F.I.Sh", "Telefon", "Sinf", "Urinishlar", "Sana"], rows, "Userlar export")
...
def export_users_pdf() -> Path:
    ...
    for idx, row in enumerate(DB.list_users(), start=1):
        rows.append([idx, row["tg_user_id"], row["username"] or "-", row["full_name"] or "-", row["phone"] or "-", grade_label(row["target_grade"]), row["attempts_count"], row["registered_at"]])
    return build_pdf_file("users_export.pdf", "Userlar export", ["#", "TG ID", "Username", "F.I.Sh", "Telefon", "Sinf", "Urinishlar", "Sana"], rows)
...
async def show_subject_hub(message: Message, state: FSMContext, subject: str, category: str) -> None:
    ...
    await state.update_data(selected_subject=subject, selected_category=category, selected_grade=grade)
...
async def start_handler(message: Message, state: FSMContext) -> None:
    ...
    if user and user["full_name"] and user["phone"]:
        await tracked_answer(message, f"{SETTINGS.bot_brand_name} ga xush kelibsiz.", reply_markup=main_menu(message.from_user.id))
        return
...
async def admin_handler(message: Message, state: FSMContext) -> None:
    ...
    if not is_admin(message.from_user.id):
        ...
        return
    await state.clear()
    await tracked_answer(message, "🛠 Admin panel ochildi.", reply_markup=admin_menu())
...
async def registration_contact_handler(message: Message, state: FSMContext) -> None:
    ...
    await tracked_answer(message, "Sinfingizni tanlang:", reply_markup=grade_keyboard())
...
async def registration_grade_handler(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await tracked_answer(message, f"Ro'yxatdan o'tish tugadi. Siz: {grade_label(grade)}", reply_markup=main_menu(message.from_user.id))
...
async def admin_exam_title(message: Message, state: FSMContext) -> None:
    ...
    await tracked_answer(message, "Test qaysi sinf uchun? 9 / 11 / ALL", reply_markup=ReplyKeyboardRemove())
...
async def admin_exam_grade(message: Message, state: FSMContext) -> None:
    await track_incoming(message)
    grade = normalize_grade_value(message.text or "")
    if not grade:
        await tracked_answer(message, "Faqat 9, 11 yoki ALL yuboring.")
        return
    await state.update_data(exam_grade=grade)
    await state.set_state(AdminStates.waiting_exam_category)
    await tracked_answer(message, "Kategoriya tanlang yoki yozing:\n📐 Aniq fanlar\n🧪 Tabiiy fanlar\n🏛 Ijtimoiy fanlar")
...
async def admin_exam_category(message: Message, state: FSMContext) -> None:
    await track_incoming(message)
    category = normalize_category(message.text or "")
    if not category:
        await tracked_answer(message, "Kategoriya noto'g'ri. Tayyor variantlardan birini yuboring.")
        return
    await state.update_data(exam_category=category)
    await state.set_state(AdminStates.waiting_exam_subject)
    await tracked_answer(message, "Fan nomini yuboring:\n" + "\n".join(CATEGORY_SUBJECTS[category]))
...
async def admin_exam_subject(message: Message, state: FSMContext) -> None:
    await track_incoming(message)
    subject = normalize_subject(message.text or "")
    data = await state.get_data()
    category = data.get("exam_category", "")
    if not subject or subject not in CATEGORY_SUBJECTS.get(category, []):
        await tracked_answer(message, "Fan noto'g'ri yoki kategoriya bilan mos emas. Qayta yuboring.")
        return
...
async def admin_exam_description(message: Message, state: FSMContext) -> None:
    ...
    await tracked_answer(message, "Bu test bir martalik tekin testmi? Ha / Yo'q")
...
async def admin_exam_free_flag(message: Message, state: FSMContext) -> None:
    ...
    if raw in {"ha", "yes", "1"}:
        is_free = True
    elif raw in {"yo'q", "yoq", "no", "0"}:
        is_free = False
    else:
        await tracked_answer(message, "Faqat Ha yoki Yo'q yuboring.")
        return
...
async def admin_exam_instructions(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await tracked_answer(message, f"✅ Test qo'shildi. ID: {exam_id}", reply_markup=admin_menu())
...
async def admin_delete_exam(message: Message, state: FSMContext) -> None:
    ...
    if not raw.isdigit():
        await tracked_answer(message, "Faqat ID yuboring.")
        return
    ok = DB.deactivate_exam(int(raw))
    await state.clear()
    await tracked_answer(message, "✅ O'chirildi." if ok else "ID topilmadi.", reply_markup=admin_menu())
...
async def admin_code_exam_id(message: Message, state: FSMContext) -> None:
    await track_incoming(message)
    raw = compact_spaces(message.text or "")
    if not raw.isdigit() or not DB.get_exam(int(raw)):
        await tracked_answer(message, "To'g'ri exam ID yuboring.")
        return
    await state.update_data(code_exam_id=int(raw))
    await state.set_state(AdminStates.waiting_code_mode)
    await tracked_answer(message, "1 - bitta kod\n2 - ko'p kod")
...
async def admin_code_mode(message: Message, state: FSMContext) -> None:
    ...
    if raw == "2":
        ...
        return
    await tracked_answer(message, "Faqat 1 yoki 2 yuboring.")
...
async def admin_single_code(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await tracked_answer(message, f"{'✅ Yaratildi' if ok else 'Bu kod mavjud'}: {code}", reply_markup=admin_menu())
...
async def admin_export_users(message: Message, state: FSMContext) -> None:
    ...
    raw = compact_spaces(message.text or "")
    if raw not in {"1", "2"}:
        await tracked_answer(message, "1 - XLSX, 2 - PDF")
        return
...
async def admin_broadcast(message: Message, state: FSMContext) -> None:
    ...
    for user in users:
        try:
            ...
            continue
    await state.clear()
    await tracked_answer(message, f"✅ Xabar yuborildi: {sent_count} ta user", reply_markup=admin_menu())
...
async def text_router(message: Message, state: FSMContext) -> None:
    ...
    if text == BTN_HOME:
        await state.clear()
        await tracked_answer(message, "🏠 Asosiy menu", reply_markup=main_menu(message.from_user.id))
        return
    ...
    if text == BTN_BACK:
        ...
        if data.get("selected_subject"):
            ...
            if data.get("from_subjects"):
                await tracked_answer(message, "📚 Fanlar bo'limi", reply_markup=subjects_keyboard())
            else:
                await tracked_answer(message, "🎯 Full mock bo'limi", reply_markup=categories_keyboard())
            return
        if data.get("selected_category"):
            await state.update_data(selected_category="")
            await tracked_answer(message, "🎯 Full mock bo'limi", reply_markup=categories_keyboard())
            return
        await tracked_answer(message, "⬅️ Orqaga qaytildi.", reply_markup=main_menu(message.from_user.id))
        return
    ...
    if text == BTN_FULL_MOCK:
        await state.update_data(from_subjects=False, selected_subject="", selected_category="", selected_grade=user["target_grade"])
        await tracked_answer(message, "🎯 Full mock bo'limi. Kategoriyani tanlang:", reply_markup=categories_keyboard())
        return

    if text == BTN_SUBJECTS:
        await state.update_data(from_subjects=True, selected_subject="", selected_category="", selected_grade=user["target_grade"])
        await tracked_answer(message, "📚 Fanlar bo'limi. Fanni tanlang:", reply_markup=subjects_keyboard())
        return

    if text in CATEGORY_SUBJECTS:
        await state.update_data(from_subjects=False, selected_category=text, selected_subject="", selected_grade=user["target_grade"])
        rows = []
        subs = CATEGORY_SUBJECTS[text]
        for i in range(0, len(subs), 2):
            rows.append(subs[i:i+2])
        await tracked_answer(message, f"{text} bo'limi. Fanni tanlang:", reply_markup=back_keyboard(rows))
        return
    ...
    if text == BTN_ADMIN:
        if not is_admin(message.from_user.id):
            ...
            return
        await state.clear()
        await tracked_answer(message, "🛠 Admin panel", reply_markup=admin_menu())
        return
    ...
    if text == BTN_ADMIN_TO_USER:
        await state.clear()
        await tracked_answer(message, "👥 User menu ochildi.", reply_markup=main_menu(message.from_user.id))
        return
    ...
    if text == BTN_EXAM_LIST:
        ...
        if not exams:
            ...
            return
        await tracked_answer(message, "📋 Faol testlar:\n" + "\n".join(format_exam_line(exam) for exam in exams[:100]), reply_markup=admin_menu())
        return
    ...
    if text == BTN_EXAM_DELETE:
        ...
        await tracked_answer(message, "O'chirish uchun ID yuboring:\n" + "\n".join(format_exam_line(exam) for exam in exams[:100]), reply_markup=admin_menu())
        return
    ...
    if text == BTN_CODE_CREATE:
        ...
        await tracked_answer(message, "Qaysi test uchun kod yaratiladi? ID yuboring:\n" + "\n".join(format_exam_line(exam) for exam in exams[:100]), reply_markup=admin_menu())
        return
    ...
    if text == BTN_EXPORT_USERS:
        ...
        await tracked_answer(message, "1 - XLSX\n2 - PDF", reply_markup=admin_menu())
        return
    ...
    await tracked_answer(message, "Admin tugmalaridan foydalaning.", reply_markup=admin_menu())
